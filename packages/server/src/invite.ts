...
const ipWindow = new Map<string, { n: number; reset: number }>();
function rateLimitIp(ip: string, limit = 10, windowMs = 60_000): boolean {
  const now = Date.now();
  const e = ipWindow.get(ip);
  if (!e || now > e.reset) { ipWindow.set(ip, { n: 1, reset: now + windowMs }); return true; }
  e.n++;
  return e.n <= limit;
}
...
const adminFail = new Map<string, { n: number; reset: number }>();
const ADMIN_FAIL_LIMIT = 5;
const ADMIN_FAIL_WINDOW_MS = 60_000;

export function adminGate(req: Request, res: Response): boolean {
  ...
  if (adminOk(req)) {
    if (hasHdr) adminFail.delete(ip);
    return true;
  }
  if (hasHdr) {
    if (!e || Date.now() >= e.reset) adminFail.set(ip, { n: 1, reset: Date.now() + ADMIN_FAIL_WINDOW_MS });
    else e.n++;
  }
...
export function inviteRouter(): Router {
  ...
  r.post("/api/shortlink", async (req: Request, res: Response) => {
    ...
    if (!token) {
      ...
      try { exp = JSON.parse(Buffer.from(token.split(".")[1], "base64").toString("utf8")).exp || 0; }
  ...
  r.get("/i/:code", (req: Request, res: Response) => {
    if (!rateLimitIp(req.ip || "unknown")) return res.status(429).send("too many requests");
    const e = loadLinks().get(String(req.params.code).toLowerCase());
    if (!e) return res.status(404).send("not found");
    if (e.revoked) return res.status(410).send("gone");
    if (e.exp && Date.now() / 1000 > e.exp) return res.status(410).send("expired");
  ...
  r.post("/api/shortlink/revoke", (req: Request, res: Response) => {
    ...
    if (!isAdmin && req.body?.token !== e.jwt) return res.status(401).json({ error: "unauthorized" });
    e.revoked = true;
    links.set(code, e);
    saveLinks(links);
